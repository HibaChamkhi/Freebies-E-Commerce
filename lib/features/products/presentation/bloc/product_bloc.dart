  ...
  ProductBloc({
    ...
    on<ProductEvent>((event, emit) async {
      ...
      if (event is GetAllProductsEvent) {
        ...
        final failureOrSuggestedProducts = await getProductsUseCase(ProductFetchType.all,null);
      ...
      if (event is SearchProductEvent) {
        ...
        final failureOrSuggestedProducts = await getProductsUseCase(ProductFetchType.all,event.query);
        failureOrSuggestedProducts.fold(
            (suggestedOrFailure) => emit(state.copyWith(
                  messages: mapFailureToMessage(suggestedOrFailure),
                  productsStatus: ProductsStatus.error,
      ...
      if (event is GetNewProductsEvent) {
        ...
        final failureOrSuggestedProducts = await getProductsUseCase(ProductFetchType.newProducts,null);
        failureOrSuggestedProducts.fold(
                (suggestedOrFailure) => emit(state.copyWith(
              messages: mapFailureToMessage(suggestedOrFailure),
              productsStatus: ProductsStatus.error,
      ...
      if (event is GetProductsWithSpecialOffersEvent) {
        ...
        final failureOrSuggestedProducts = await getProductsUseCase(ProductFetchType.productsWithSpecialOffers,null);
        failureOrSuggestedProducts.fold(
                (suggestedOrFailure) => emit(state.copyWith(
              messages: mapFailureToMessage(suggestedOrFailure),
              productsStatus: ProductsStatus.error,
      ...
      if (event is GetBestSellersEvent) {
        ...
        final failureOrSuggestedProducts = await getProductsUseCase(ProductFetchType.bestSellers,null);
        failureOrSuggestedProducts.fold(
                (suggestedOrFailure) => emit(state.copyWith(
              messages: mapFailureToMessage(suggestedOrFailure),
              productsStatus: ProductsStatus.error,
      ...
      if (event is GetTopRatedProductsEvent) {
        ...
        final failureOrSuggestedProducts = await getProductsUseCase(ProductFetchType.topRated,null);
        failureOrSuggestedProducts.fold(
                (suggestedOrFailure) => emit(state.copyWith(
              messages: mapFailureToMessage(suggestedOrFailure),
              productsStatus: ProductsStatus.error,
